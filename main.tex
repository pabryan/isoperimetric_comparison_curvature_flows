...
Here we take \(X = -\ric + \varphi g\) where \(\phi \in C^{\infty}(M)\) is a smooth function. Particular examples are,
\begin{align*}
\varphi &= \frac{1}{n} \bar{R} = \frac{1}{n\abs{M}} \int_M R \mu & \text{Normalised Ricci flow} \\
\varphi &= \frac{1}{n} \bar{R} - \operatorname{div} V & \text{Ricci flow with metric torsion} \\
\varphi &= \frac{1}{n} \abs{F}^2 & \text{Ricci Yang-Mills flow}
\end{align*}

In the latter two situations, the vector field \(V\) and the two form \(F\) are supposed to satisfy evolution equations, dependent on \(g\) and hence coupled to the Ricci flow.

For the Ricci flow with metric torsion, \(V\) is a vector field generating a metric compatible connection with torsion. That is, let \(\nabla\) denote the Levi-Civita connection and define a connection \(\tilde{\nabla}\) to be the unique metric compatible connection with torsion tensor,
\[
